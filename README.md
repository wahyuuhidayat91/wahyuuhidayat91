<h1 align="center">Hi, I'm Wahyu Hidayat 👋</h1>

### 🚀 About Me
- 💻 QA Engineer | Automation & Manual Testing  
- ⚙️ Familiar with: Postman, Katalon, Cypress,  
- ☕ Coffee enthusiast ☕  
- 📍 Based in Bogor - Indonesia  

---

### 🧩 Tech Stack

#### 🖥️ Programming & Scripting
<p>
  <img src="https://skillicons.dev/icons?i=html,css" />
</p>

#### 🧪 Testing Tools
<p>
  <img src="https://skillicons.dev/icons?i=postman" />
  <img src="https://img.shields.io/badge/-Katalon-1DDB8D?logo=katalon&logoColor=white" />
  <img src="https://img.shields.io/badge/-Cypress-17202C?logo=cypress&logoColor=white" />
</p>

#### ⚙️ DevOps & Cloud
<p>
  <img src="https://skillicons.dev/icons?i=docker,github,git,linux" />
</p>

---

### 📂 Featured Projects

#### 🔹 **API Automation with Postman**
Repository: [link soon]  
A collection of automated API tests with environment control and report generation.

#### 🔹 **Vue.js Frontend Playground**
Repository: [link soon]  
UI components, layouts, and micro interactions using Vue.

#### 🔹 **Go API Service Example**
Repository: [link soon]  
Simple REST API with clean architecture.

---

### 📫 Contact Me
- 📧 Email: **wahyuuhidayat091@gmail.com**
- 💼 LinkedIn: [*Wahyu Hidayat*](https://www.linkedin.com/in/wahyu-hidayat-5584111b4/)

---

<p align="center">⭐️ Feel free to check my repositories and give a star!</p>
